...
**Frontend PPTX** helps non-designers create beautiful web presentations without knowing CSS or JavaScript. It uses a "show, don't tell" approach: instead of asking you to describe your aesthetic preferences in words, it generates visual previews and lets you pick what you like.
...
Then use it by typing `/frontend-pptx` in Claude Code.
...
mkdir -p ~/.claude/skills/frontend-pptx/scripts

# Copy all files (or clone this repo directly)
cp SKILL.md STYLE_PRESETS.md viewport-base.css html-template.md animation-patterns.md ~/.claude/skills/frontend-pptx/
cp scripts/extract-pptx.py ~/.claude/skills/frontend-pptx/scripts/
```
...
Created by [@LinYi-7](https://github.com/LinYi-7), adapted from the original `frontend-slides` concept with Claude Code.
